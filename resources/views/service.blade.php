                ...
                <div class="col-lg-3 col-12 mg-top-30">
                    <form action="">
                        <div class="inflanar-service-sidebar">
                            ...
                            <div class="inflanar-service-sidebar__single">
                                ...
                                <div class="inflanar-search-form inflanar-search-form__service mg-top-10">
                                    <div class="inflanar-search-form__form">
                                        <input type="text" placeholder="{{ __('admin.Search') }}" name="search"
                                            value="{{ request()->get('search') }}">
                                        <button type="submit" class="inflanar-btn inflanar-btn--search"><img
                                                src="{{ asset('frontend/img/in-search-white.svg') }}"></button>
                                    </div>
                                    ...
                                    @if (request()->has('categories'))
                                        @foreach ($categories as $category)
                                            ...
                                            <div class="form-group inflanar-form-checkbox mg-top-15">
                                                <input {{ $is_request == true ? 'checked' : '' }} type="checkbox"
                                                    id="cat{{ $category->id }}" name="categories[]"
                                                    value="{{ $category->slug }}">
                                                <label class="inflanar-form-label"
                                                    for="cat{{ $category->id }}">{{ $category->name }}
                                                    <span>({{ $category->total_service }})</span></label>
                                            </div>
                                        @endforeach
                                    @else
                                        @foreach ($categories as $category)
                            ...
                            <div class="inflanar-service-sidebar__single mg-top-50">
                                <div class="inflanar-service-sidebar__checkboxd">
                                    <h4 class="inflanar-service-sidebar__title m-0">{{ __('admin.Select Platform') }}</h4>

                                    @if (request()->has('platforms'))
                                        @foreach ($platforms as $platform)
                                            @php
                                                $is_request = false;
                                                foreach (request()->get('platforms') as $request_platform) {
                                                    if ($request_platform == $platform->slug) {
                                                        $is_request = true;
                                                    }
                                                }
                                            @endphp
                                            ...
                                            <div class="form-group inflanar-form-checkbox mg-top-15">
                                                <input {{ $is_request == true ? 'checked' : '' }} type="checkbox"
                                                    ...
                                                    value="{{ $platform->slug }}">
                                                <label class="inflanar-form-label"
                                                    for="platform{{ $platform->id }}">{{ $platform->name }}
                                                    <span>({{ $platform->total_service }})</span></label>
                                            </div>
                                        @endforeach
                                    @else
                                        @foreach ($platforms as $platform)
                                    ...
                                    <div class="price-filter-inner">
                                        ...
                                        <div class="price_slider_amount">
                                            <div class="label-input">
                                                <span>{{ __('admin.Price') }}:</span><input type="text" id="amount"
                                                    readonly>
                                            </div>
                            ...
                            <button type="submit"
                                class="inflanar-btn mg-top-20"><span>{{ __('admin.Search Here') }}</span></button>
                ...
                <div class="row">
                                ...
                                <div class="inflanar-service__body">
                                    ...
                                    <div class="inflanar-service__author">
                                        ...
                                        <div class="inflanar-service__author--rating">
                                            ...
                                            <div class="inflanar-service__author--label">({{ $service->total_review }})
                                            </div>
                                        </div>
                    ...
                    @empty
                        <div class="col-12  text-center ">
                            ...
                            <p class="mb-4">{{ __('admin.Whoops... this information is not available for a moment') }}
                            </p>
                            <a href="{{ route('services') }}"
                                class="inflanar-btn inflanar-btn--header"><span>{{ __('admin.Back to Service') }}</span></a>
                        </div>
                    @endforelse
