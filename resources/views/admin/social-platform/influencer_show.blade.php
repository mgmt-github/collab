    ...
    <div class="main-content">
        <section class="section">
            ...
            <div class="section-body">
                <a href="{{ route('admin.influencer-list') }}" class="btn btn-primary"><i class="fas fa-list"></i>
                    {{ __('admin.Influencer List') }}</a>
                <div class="row mt-5">
                    <div class="col-md-3">
                    ...
                    <div class="col-12 col-md-12 col-lg-5">
                        <div class="card profile-widget">
                            <div class="profile-widget-header">
                                @if ($influencer->image)
                                    <img alt="image" src="{{ asset($influencer->image) }}"
                                ...
                                <div class="profile-widget-items">
                                    <div class="profile-widget-item">
                                        ...
                                        <div class="profile-widget-item-value">
                                            {{ $influencer->created_at->format('d M Y') }}</div>
                                    </div>
                                    <div class="profile-widget-item">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($influencer->status == 'enable')
                                                    <a href="javascript:;"
                                                        onclick="manageCustomerStatus({{ $influencer->id }})">
                            ...
                            <div class="col-12">
                                <div class="card">
                                    ...
                                    <div class="card-body text-center">
                                        <div class="row">
                                            <div class="col-12">
                                                <a target="_blank" href="{{ route('influencer', $influencer->username) }}"
                                                    class="btn btn-success btn-block btn-lg my-2">{{ __('admin.Go to Influencer Front Page') }}</a>
                                            </div>

                                            <div class="col-12">
                                                <a href="{{ route('admin.review-list', ['influencer_id' => $influencer->id]) }}"
                                                    class="btn btn-primary btn-block btn-lg my-2">{{ __('admin.Influencer Reviews') }}</a>
                                            </div>

                                            <div class="col-12">
                                                <a href="{{ route('admin.send-email-to-influencer', $influencer->id) }}"
                    ...
                    <div class="col-12 col-md-12 col-lg-7">
                        <div class="card">
                            <form method="post" class="needs-validation" novalidate=""
                                action="{{ route('admin.influencer-update', $influencer->id) }}">
                                @method('put')
                                @csrf
                                <div class="card-header">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-6">
                                            ...
                                            <input type="text" class="form-control" value="{{ $influencer->name }}"
                                                name="name">
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Designation') }} <span
                                                    class="text-danger">*</span></label>
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->designation }}" name="designation">
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Email') }} <span class="text-danger">*</span></label>
                                            <input type="email" class="form-control" value="{{ $influencer->email }}"
                                                name="email" readonly>
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Phone') }} <span class="text-danger">*</span></label>
                                            <input type="text" class="form-control" value="{{ $influencer->phone }}"
                                                name="phone">
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Total Follower') }} <span
                                                    class="text-danger">*</span></label>
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->total_follower }}" name="total_follower">
                                        </div>
                                        <div class="form-group col-6">
                                            ...
                                            <select name="platform[]" class="select2" multiple>
                                                @foreach ($platform as $item)
                                                    <option value="{{ $item->id }}"
                                                        @if (in_array($item->platform, json_decode($influencer->platform ?? '[]', true))) selected @endif>
                                                        {{ $item->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group col-12">
                                            ...
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->school_name }}" name="school_name">
                                        </div>

                                        <div class="form-group col-12">
                                            <label>{{ __('admin.School passing year') }}</label>
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->school_year }}" name="school_year">
                                        </div>
                                        ...
                                        <div class="form-group col-12">
                                            ...
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->varsity_name }}" name="varsity_name">
                                        </div>

                                        <div class="form-group col-12">
                                            <label>{{ __('admin.University passing year') }}</label>
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->varsity_year }}" name="varsity_year">
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Facebook') }} </label>
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->facebook }}" name="facebook">
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Twitter') }} </label>
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->twitter }}" name="twitter">
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Instagram') }} </label>
                                            <input type="text" class="form-control"
                                                value="{{ $influencer->instagram }}" name="instagram">
                                        </div>

                                        <div class="form-group col-6">
                                            <label>{{ __('admin.Tiktok') }} </label>
                                            <input type="text" class="form-control" value="{{ $influencer->tiktok }}"
                                                name="tiktok">
                                        </div>
